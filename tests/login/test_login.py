...
def fake_login_flow(monkeypatch):
    class MockClient:
        call_count = -1
        ...
        def handle_call(self, **kwargs):
            ...
            assert kwargs == self.arg_sets[MockClient.call_count]
            return self.return_sets[MockClient.call_count]
    ...
    monkeypatch.setattr("kshalopy.login.login.boto3.client", MockClient)
...
def test_login_flow(monkeypatch):
    assert fake_login_flow(monkeypatch).access_token == "fake_access_token"
